...
(function () {
    ...
    const I18N = {
      en: {
        ...
        'watchInspector.title':     'Watch Inspector',
        'watchInspector.subtitle':  'View stored vs live content for each watch',
        'watchOverview.title':      'Watch Overview',
        'watchOverview.subtitle':   'All watches across pages',
        ...
        'badge.titleOptions':       'Click for options',
        ...
        'ctx.watchElement':         '🔍 Watch this element',
        'ctx.reSnapshot':           '🔄 Re-snapshot',
        'ctx.removeWatch':          '🗑 Remove watch',
        'ctx.inspect':              '🔍 Inspect',
        'ctx.alertContent':         'Alert when text changes',
        'ctx.alertStyle':           'Alert when CSS changes',
        'ctx.alertBoth':            'Monitor text and styling',
        ...
        'inspector.styleChanges':   'Style Changes',
        ...
        'inspector.navigateFirst':  'Navigate to this page to see live content',
        'inspector.elementMissing': '⚠ Element not found on page',
        'inspector.snapshot':       'Snapshot: {time}',
        'inspector.showFullDiff':   'Show full diff ({count} chars)',
        'inspector.reSnapshotTip':  'Navigate to this page to re-snapshot',
        'inspector.reSnapshotLocal':'Re-snapshot to current values',
        ...
        'overview.removeConfirm':   '🗑 Click again to confirm',
        ...
        // TTS speech
        'speech.missingElements':   'Warning: {count} watched element not found on {page}.|Warning: {count} watched elements not found on {page}.',
        'speech.watchChanged':      'Watch changed on {page}. Content updated.',
        'speech.multiChanged':      '{count} watches changed on {page}.',

        // Alert messages
        'alert.missingElements':    '{count} watched element not found!|{count} watched elements not found!',
        'alert.watchesChanged':     '{count} watch changed!|{count} watches changed!',
        ...
        'menu.setFontSize':         'Set badge font size',
        'menu.setPosition':         'Set badge position',
        'menu.setInterval':         'Set refresh interval',
        'menu.setShortcut':         'Set keyboard shortcut',
        ...
        // Key aliases (code uses these shorter/alternate names)
      ...
      es: {
        ...
        'watchInspector.title':     'Inspector de Watches',
        'watchInspector.subtitle':  'Ver contenido almacenado vs actual para cada watch',
        'watchOverview.title':      'Resumen de Watches',
        'watchOverview.subtitle':   'Todos los watches en todas las páginas',
        ...
        'btn.reSnapshot':           '🔄 Re-capturar',
        'btn.remove':               '🗑 Eliminar',
        'btn.openPage':             '🔗 Abrir Página',
        ...
        'badge.titleOptions':       'Haz clic para opciones',
        ...
        'ctx.watchElement':         '🔍 Monitorizar este elemento',
        'ctx.reSnapshot':           '🔄 Re-capturar',
        'ctx.removeWatch':          '🗑 Eliminar watch',
        'ctx.inspect':              '🔍 Inspeccionar',
        'ctx.alertContent':         'Alertar cuando el texto cambie',
        'ctx.alertStyle':           'Alertar cuando el CSS cambie',
        'ctx.alertBoth':            'Monitorizar texto y estilo',
        ...
        'inspector.styleChanges':   'Cambios de Estilo',
        ...
        'inspector.navigateFirst':  'Navega a esta página para ver el contenido actual',
        'inspector.elementMissing': '⚠ Elemento no encontrado en la página',
        'inspector.snapshot':       'Captura: {time}',
        'inspector.showFullDiff':   'Mostrar diff completo ({count} caracteres)',
        'inspector.reSnapshotTip':  'Navega a esta página para re-capturar',
        'inspector.reSnapshotLocal':'Re-capturar a los valores actuales',
        ...
        'overview.removeConfirm':   '🗑 Haz clic de nuevo para confirmar',
        'overview.total':           '{count} total',
        ...
        'speech.missingElements':   'Advertencia: {count} elemento monitoreado no encontrado en {page}.|Advertencia: {count} elementos monitoreados no encontrados en {page}.',
        'speech.watchChanged':      'Watch cambiado en {page}. Contenido actualizado.',
        'speech.multiChanged':      '{count} watches cambiaron en {page}.',

        'alert.missingElements':    '¡{count} elemento monitoreado no encontrado!|¡{count} elementos monitoreados no encontrados!',
        'alert.watchesChanged':     '¡{count} watch cambió!|¡{count} watches cambiaron!',
        ...
        'menu.setFontSize':         'Configurar tamaño de fuente',
        'menu.setPosition':         'Configurar posición del badge',
        'menu.setInterval':         'Configurar intervalo de actualización',
        'menu.setShortcut':         'Configurar atajo de teclado',
    ...
    const badge = h('div', {
        ...
        style: [
            ...
            'backdrop-filter:blur(4px)', 'transition:opacity .2s,box-shadow .3s,width .2s ease',
            'overflow:hidden', 'pointer-events:auto'
        ].join(';')
    });

    const badgeContent = h('div', { style: 'padding:6px 10px;display:flex;align-items:center;justify-content:center;gap:6px;white-space:nowrap' });
    ...
    const hoverInfo = h('div', { style: `max-height:0;overflow:hidden;transition:max-height .2s,padding .2s;font-size:11px;color:${T.textMuted};padding:0 10px;border-top:0px solid ${T.borderMid};white-space:nowrap;text-align:center` });
    ...
    let _badgeHoverEndHandler = null;

    badge.addEventListener('mouseenter', () => {
        // Remove any pending transitionend handler from a previous mouseleave
        if (_badgeHoverEndHandler) {
            badge.removeEventListener('transitionend', _badgeHoverEndHandler);
            _badgeHoverEndHandler = null;
        }
    ...
    badge.addEventListener('mouseleave', () => {
        ...
        badge.addEventListener('transitionend', _badgeHoverEndHandler);
    });
        ...
        build: (panel) => {
            ...
            [
                ...
                const btn = makeOptionBtn(label, key === current, () => {
                    ...
                    applyFontSize();
                    Modal.open('settings');
                    showToast(t('toast.fontSizeSet', { label }));
                });
    ...
    Modal.define('theme-picker', {
        ...
        build: (panel) => {
            ...
            [
                { key: 'dark', label: t('theme.dark'), desc: t('theme.dark.desc') },
                { key: 'light', label: t('theme.light'), desc: t('theme.light.desc') },
                { key: 'minimal', label: t('theme.minimal'), desc: t('theme.minimal.desc') },
                { key: 'highContrast', label: t('theme.highContrast'), desc: t('theme.highContrast.desc') }
            ].forEach(({ key, label, desc }) => {
                const theme = THEMES[key];
                const btn = makeOptionBtn(label, key === current, () => {
                    applyTheme(key);
                    Modal.open('settings');
                    showToast(t('toast.themeSet', { label }));
                }, { subtitle: desc });
                // Live badge preview
    ...
    Modal.define('position-picker', {
        ...
        build: (panel) => {
            ...
            ['top-left', 'top-center', 'top-right', 'bottom-left', 'bottom-center', 'bottom-right'].forEach(choice => {
                const labels = getCornerLabels();
                const btn = makeOptionBtn(labels[choice], choice === current, () => {
                    Config.set('corner', choice);
                    applyCorner();
                    Modal.open('settings');
                    showToast(t('toast.positionSet', { label: getCornerLabels()[choice] }));
                });
    ...
    Modal.define('language-picker', {
        ...
        build: (panel, ctx) => {
            const current = Config.get('language');

            LANGUAGE_OPTIONS.forEach(({ code, label, desc }) => {
                const isActive = code === current;
                panel.appendChild(makeOptionBtn(label, isActive, () => {
                    Config.set('language', code);
                    showToast(t('toast.languageSet', { label }));
                    ctx.closeAll();
                }, { subtitle: desc }));
            });
        }
    ...
    Modal.define('webhook-edit', {
        ...
        build: (panel, ctx) => {
            ...
            testBtn.addEventListener('click', async () => {
                if (!currentUrl) { showToast(t('toast.enterUrl')); urlInput.style.borderColor = T.err; return; }
                try { new URL(currentUrl); } catch { showToast(t('toast.invalidUrl')); urlInput.style.borderColor = T.err; return; }
            ...
            saveBtn.addEventListener('click', () => {
                if (!currentUrl) { showToast(t('toast.enterUrlShort')); urlInput.style.borderColor = T.err; return; }
                try { new URL(currentUrl); } catch { showToast(t('toast.invalidUrl')); urlInput.style.borderColor = T.err; return; }
                urlInput.style.borderColor = '';
            ...
            watches.forEach((w) => {
                ...
                btn.addEventListener('click', () => {
                    ...
                    if (remaining === 0) {
                        ...
                        showToast(t('toast.allWatchesRemoved'));
                    } else {
                        showToast(t('toast.watchRemoved', { count: remaining }));
                        if (Modal.hasInStack('settings')) Modal.open('settings');
                        else ctx.closeAll();
                    }
                });
        ...
        build: (panel, ctx) => {
            ...
            [
                ...
                const btn = makeOptionBtn(label, false, () => {
                    ...
                    showToast(t('toast.watching', { mode: key, selector: _truncate(selector, 40), count }));
    ...
    Modal.define('badge-menu', {
        ...
        build: (panel, ctx) => {
            ...
            panel.appendChild(makeOptionBtn(t('btn.stop'), false, () => {
                stopRefresh(); ctx.closeAll(); showToast(t('toast.refreshStopped'));
    ...
    Modal.define('settings', {
        ...
        build: (panel, ctx) => {
            ...
            panel.appendChild(makeOptionBtn(t('setting.interval'), false, () => ctx.push('interval-picker'),
                { value: formatSeconds(getInterval()), subtitle: t('setting.interval.sub') }));

            // Watches section
            addSection(panel, `${t('section.watches')}${watchCount > 0 ? ` (${watchCount})` : ''}`);

            panel.appendChild(makeOptionBtn(t('setting.addWatch'), false, () => { ctx.closeAll(); startElementPicker(); },
            ...
            panel.appendChild(makeOptionBtn(t('setting.watchOverview'), false, () => ctx.push('watch-overview'),
            ...
            if (watchCount > 0) {
                panel.appendChild(makeOptionBtn(t('setting.inspectWatches'), false, () => ctx.push('watch-inspector'),
                    { value: `${watchCount}`, subtitle: t('setting.inspect.sub') }));
                panel.appendChild(makeOptionBtn(t('setting.removeWatches'), false, () => ctx.push('clear-watch'),
            ...
            moreWrapper.appendChild(h('div', { class: 'ar-section-hdr ar-section-hdr-sep', text: t('section.badge') }));
            moreWrapper.appendChild(makeOptionBtn(t('setting.badgeStyle'), false, () => ctx.push('badge-style'),
                { value: badgeStyleValue, subtitle: t('setting.badgeStyle.sub') }));
            moreWrapper.appendChild(makeOptionBtn(t('setting.theme'), false, () => ctx.push('theme-picker'),
                { value: themeLabel, subtitle: t('setting.theme.sub') }));

            // General section
            moreWrapper.appendChild(h('div', { class: 'ar-section-hdr ar-section-hdr-sep', text: t('section.general') }));
            moreWrapper.appendChild(makeOptionBtn(t('setting.hotkey'), false, () => ctx.push('hotkey-picker'),
            ...
            moreWrapper.appendChild(makeOptionBtn(t('setting.alertMode'), false, () => ctx.push('alert-mode-picker'),
            ...
            moreWrapper.appendChild(makeOptionBtn(t('setting.language'), false, () => ctx.push('language-picker'),
            ...
            moreWrapper.appendChild(makeOptionBtn(t('setting.webhook'), false, () => ctx.push('webhook-list'),
            ...
            function renderDetail() {
                ...
                const refreshBtn = h('button', {
                    ...
                    onclick: () => {
                        ...
                        if (!reSnapshotWatch(w)) { showToast(t('toast.elementNotFound')); return; }
                        watches = getWatches();
                        renderDetail();
                        buildSidebar();
                ...
                const deleteBtn = h('button', {
                    ...
                    onclick: () => {
                        ...
                        buildSidebar();
                        renderDetail();
                        showToast(t('toast.watchRemoved'));
                    }
                ...
                filteredDomains.forEach(domain => {
                    ...
                    if (domainCount > 1) {
                        ...
                        removeAllBtn.addEventListener('click', () => {
                            ...
                            showToast(t('overview.removedToast', { count: domainCount, domain }));
                            refreshData();
                            renderList();
                        });
    ...
    function showQuickWatchMenu(x, y, targetEl, selector) {
        ...
        header.appendChild(h('div', { class: 'ar-ctx-title', text: isWatched ? t('ctx.alreadyWatching') : t('ctx.watchThis') }));
        ...
        if (isWatched) {
            ...
            menu.appendChild(h('button', {
                ...
                onclick: () => {
                    if (!reSnapshotWatch(w)) { showToast(t('toast.elementNotFound')); dismissQuickWatch(); return; }
                    dismissQuickWatch();
                    showToast(t('toast.snapshotUpdated'));
                }
            }));
            ...
            menu.appendChild(h('button', {
                ...
                onclick: () => {
                    removeWatch(w.id);
                    dismissQuickWatch();
                    showToast(t('toast.watchRemovedSel', { selector: _truncate(selector) }));
                }
            }));
    ...
    async function checkForChanges() {
        ...
        if (missing.length > 0) {
            stopRefresh();
            speak(t('speak.missingWarning', { count: missing.length, page: _pageLabel() }));
            startAlert();
            showToast(t('toast.missingElements', { count: missing.length }));
            return;
        }
        ...
        if (allChanges.length > 0) {
            ...
            if (allChanges.length === 1) {
                summary = t('speak.watchChanged', { page: _pageLabel() });
                speak(summary);
            } else {
                summary = t('speak.watchesChanged', { count: allChanges.length, page: _pageLabel() });
                speak(summary);
            }
            startAlert();
            showToast(t('toast.watchesChanged', { count: allChanges.length }));
    ...
    function runTests() {
        ...
        const grouped = groupWatchesByDomain(testWatches);
        ...
        const malformed = groupWatchesByDomain([{ selector: '.x', _urlKey: 'not-a-url' }]);
