...
(function () {
    ...
    function makeOptionBtn(label, isActive, onClick, opts = {}) {
        ...
        if (opts.subtitle || opts.value) {
            ...
            top.appendChild(h('span', { text: label }));
    ...
    function startAlert() { stopAlert(); playBeep(); alertIntervalId = setInterval(playBeep, 2000); }
    function stopAlert() { if (alertIntervalId !== null) { clearInterval(alertIntervalId); alertIntervalId = null; } }
        ...
        build: (panel, ctx) => {
            ...
            function buildSidebar() {
                ...
                watches.forEach((w, i) => {
                    ...
                    const item = h('button', { class: `ar-sidebar-item${i === current ? ' ar-active' : ''}` },
                    ...
                    item.addEventListener('click', () => { current = i; renderDetail(); updateSidebarSelection(); });
            ...
            function updateSidebarSelection() {
                sidebarItems.forEach((el, i) => {
                    el.className = `ar-sidebar-item${i === current ? ' ar-active' : ''}`;
                });
            }
            ...
            function renderDetail() {
                ...
                headerRow.appendChild(h('span', {
                    ...
                    style: `font-weight:bold;font-size:13px;color:${MODE_COLORS[w.mode] || T.cyan}`
                }));
    ...
    function showQuickWatchMenu(x, y, targetEl, selector) {
        ...
        const watches = getWatches();
        const existingIdx = watches.findIndex(w => w.selector === selector);
        const isWatched = existingIdx >= 0;
