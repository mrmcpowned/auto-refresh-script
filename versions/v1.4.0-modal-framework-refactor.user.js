...
(function () {
    ...
    function openBadgeMenu() {
        if (Modal.isOpen() && Modal.hasInStack('settings')) {
            Modal.push('badge-menu');
        } else {
            Modal.open('badge-menu');
        }
    }
    ...
    const Modal = (() => {
        ...
        function _isOpen() { return _modalOpen; }
        ...
        function _addFooter(panel, text) {
        ...
        function _show(id, extra) {
            ...
            const wasPaused = stack.length > 0;

            // If there's a current modal, hide it but keep it in stack
            if (stack.length > 0) {
            ...
            // Pause timer on first modal open
            ...
            const ctx = {
                ...
                cleanups,
                extra: extra || {},
                addFooter: (text) => _addFooter(panel, text),
            };

            def.build(panel, ctx);

            // Auto-add footer if not already added by build
            if (!def.noAutoFooter && !panel.querySelector('[data-modal-footer]')) {
                ...
                if (footerText) {
                    ...
                    f.setAttribute('data-modal-footer', '');
                    f.textContent = footerText;
                    f.style.cssText = `margin-top:12px;padding-top:8px;border-top:1px solid ${T.borderMid};text-align:center;font-size:11px;color:${T.borderLight};`;
                    panel.appendChild(f);
                }
            }
    ...
    // --- Modal Definitions: Simple Pickers ---
    ...
    Modal.define('position-picker', {
        ...
        build: (panel, ctx) => {
            ...
            grid.style.cssText = [
                ...
                'min-width:280px', `background:${T.bgDark}`, `border:1px solid ${T.border}`,
    ...
    Modal.define('clear-watch', {
        ...
        build: (panel, ctx) => {
            ...
            if (watches.length === 0) {
                ...
                msg.textContent = 'No watches to clear';
    ...
    Modal.define('badge-menu', {
        ...
        build: (panel, ctx) => {
            ...
            const footer = document.createElement('div');
            footer.textContent = isSubOfSettings ? 'Press Esc to go back' : 'Press Esc to dismiss';
            footer.style.cssText = `margin-top:12px;padding-top:8px;border-top:1px solid ${T.borderMid};text-align:center;font-size:11px;color:${T.borderLight};`;
            panel.appendChild(footer);
        }
    });
    ...
    Modal.define('watch-inspector', {
        ...
        build: (panel, ctx) => {
            ...
            let current = (ctx.extra && ctx.extra.startIndex) || 0;
