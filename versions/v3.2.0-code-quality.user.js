...
(function () {
    ...
    const WATCH_MODE_LABELS = { content: '📝 Text Content', style: '🎨 Styling', both: '📝🎨 Both' };
    const WATCH_MODE_DESCS = {
    ...
    Modal.define('clear-watch', {
        ...
        build: (panel, ctx) => {
            ...
            const modeTextLabels = WATCH_MODE_LABELS;
            ...
            if (watches.length > 1) {
                ...
                const clearBtn = makeOptionBtn('❌ Clear All', false, () => {
                    if (!confirmPending) {
                        ...
                        else clearBtn.textContent = '❌ Click again to confirm';
                        clearBtn.style.background = T.errBgDark;
                        setTimeout(() => {
                            confirmPending = false;
                            const s = clearBtn.querySelector('span');
                            if (s) s.textContent = '❌ Clear All';
                            else clearBtn.textContent = '❌ Clear All';
    ...
    Modal.define('watch-mode', {
        ...
        build: (panel, ctx) => {
            ...
            const modeDescriptions = WATCH_MODE_DESCS;
    ...
    Modal.define('watch-inspector', {
        ...
        build: (panel, ctx) => {
            ...
            const modeLabels = WATCH_MODE_LABELS;
            ...
            function renderDetail() {
                ...
                const refreshBtn = h('button', {
                    ...
                    onclick: () => {
                        ...
                        if (!reSnapshotWatch(w)) { showToast('Element not found'); return; }
                        watches = getWatches();
                        renderDetail();
                        buildSidebar();
                        showToast('Snapshot updated');
                    }
                }, h('span', { text: '🔄 ' }), h('span', { text: 'Re-snapshot', style: 'font-size:11px' }));
                ...
                }, h('span', { text: '🗑 ' }), h('span', { text: 'Remove', style: 'font-size:11px' }));
                ...
                if (isRemote && w.url) {
                    ...
                    }, h('span', { text: '🔗 ' }), h('span', { text: 'Open Page', style: 'font-size:11px' }));
    ...
    function showQuickWatchMenu(x, y, targetEl, selector) {
        ...
        if (isWatched) {
            ...
            menu.appendChild(h('button', {
                ...
                onclick: () => {
                    if (!reSnapshotWatch(w)) { showToast('Element not found'); dismissQuickWatch(); return; }
                    dismissQuickWatch();
                    showToast('Snapshot updated');
                }
            }));
            ...
            ['content', 'style', 'both'].forEach(mode => {
                const btn = h('button', {
                    class: 'ar-btn', text: WATCH_MODE_LABELS[mode],
                    onclick: () => {
    ...
    GM_registerMenuCommand('Run tests', runTests);
