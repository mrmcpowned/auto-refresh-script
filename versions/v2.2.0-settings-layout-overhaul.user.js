...
(function () {
    ...
    function buildCss() {
        ...
        .ar-toggle-track-sm{width:34px;height:18px;border-radius:9px;position:relative;transition:background .2s;flex-shrink:0}
        .ar-toggle-thumb{width:18px;height:18px;border-radius:50%;background:${T.textLight};position:absolute;top:2px;transition:left .2s;box-shadow:0 1px 3px ${T.shadow}}
        .ar-toggle-thumb-sm{width:14px;height:14px;border-radius:50%;background:${T.textLight};position:absolute;top:2px;transition:left .2s;box-shadow:0 1px 2px ${T.shadow}}
        ...
        .ar-more-link{text-align:center;font-size:12px;color:${T.accent};padding:10px 8px;cursor:pointer;border:1px dashed ${T.borderMid};border-radius:6px;margin-top:8px;transition:background .15s}
        .ar-more-link:hover{background:${T.accentFaint}}
    ...
    function getBadgeLabels() {
        const corner = GM_getValue(STORAGE_KEY_CORNER, 'bottom-right');
        const fontSize = GM_getValue(STORAGE_KEY_FONT_SIZE, 'medium');
        const opacity = GM_getValue(STORAGE_KEY_OPACITY, 1);
        return {
