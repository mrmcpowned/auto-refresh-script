...
(function () {
    ...
    function previewSpeak({ voice, rate, volume } = {}) {
        ...
        // Chrome/Edge can silently drop utterances or get stuck in a paused state
        // after cancel(). Delay + resume() workaround, with retry if speak fails.
        let attempts = 0;
        function trySpeak() {
            _previewSpeakTimer = null;
            speechSynthesis.resume(); // unstick Chrome's paused state
            speechSynthesis.speak(utter);
            // Verify speech actually started; retry once if it didn't
            if (attempts < 1) {
                attempts++;
                _previewSpeakTimer = setTimeout(() => {
                    if (!speechSynthesis.speaking && !speechSynthesis.pending) {
                        speechSynthesis.cancel();
                        _previewSpeakTimer = setTimeout(trySpeak, 100);
                    }
                }, 150);
            }
        }
    ...
    function makePreviewBtn(overrides, opts = {}) {
        ...
        btn.addEventListener('click', (e) => {
            ...
            // Reset any other active preview (cancel happens inside previewSpeak)
            if (_activePreviewBtn) {
                if (_previewSpeakTimer) { clearTimeout(_previewSpeakTimer); _previewSpeakTimer = null; }
                speechSynthesis.cancel();
            }
