...
(function () {
    ...
    function openBadgeMenu() {
        // If settings modal was open, remember so we can return to it
        const settings = document.getElementById('auto-refresh-settings');
        const cameFromSettings = !!settings;
        if (settings) { settings.remove(); modalOpen = false; }
        pauseRefresh();
        const watches = getWatches();
        const watchCount = watches.length;

        const returnTo = cameFromSettings ? openSettingsMenu : resumeRefresh;
        ...
        openModal('auto-refresh-badge-menu', 'Quick Actions', (panel, close) => {
            ...
            // Settings (only show if not already coming from settings)
            if (!cameFromSettings) {
            ...
            footer.textContent = cameFromSettings ? 'Press Esc to go back' : 'Press Esc to dismiss';
