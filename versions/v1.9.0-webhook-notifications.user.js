...
(function () {
    ...
    Modal.define('webhook-config', {
        ...
        minWidth: '380px',
        build: (panel, ctx) => {
            const config = _getWebhookConfig() || { url: '', format: 'generic', template: WEBHOOK_DEFAULT_TEMPLATE };
            ...
            saveBtn.addEventListener('click', () => {
                ...
                urlInput.style.borderColor = '';
                if (currentUrl) {
                    GM_setValue(STORAGE_KEY_WEBHOOK, { url: currentUrl, format: currentFormat, template: currentTemplate });
                    showToast('Webhook saved');
                } else {
                    GM_setValue(STORAGE_KEY_WEBHOOK, null);
            ...
            // Disable button (only when webhook is already configured)
            if (config.url) {
                panel.appendChild(h('button', {
                    class: 'ar-link-btn ar-link-btn-center',
    ...
    Modal.define('settings', {
        ...
        build: (panel, ctx) => {
            ...
            const webhookLabel = webhookConfig && webhookConfig.url ? 'Configured' : 'Off';
    ...
    async function checkForChanges() {
        ...
        for (let i = 0; i < watches.length; i++) {
            ...
            if (w.mode === 'style' || w.mode === 'both') {
                if (w.styles && diffStyles(w.styles, captureStyles(el)).length > 0) changes.push('styling');
            }
