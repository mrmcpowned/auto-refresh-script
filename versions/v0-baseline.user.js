...
(function () {
    ...
    function showToast(message) {
        const existing = document.getElementById('auto-refresh-toast');
                ...
                modes.forEach(({ key, label }) => {
                    const btn = makeOptionBtn(label, false, () => {
                        ...
                        if (key === 'content' || key === 'both') {
                            watch.content = el.textContent.trim();
                        }
    ...
    function getWatchStatus(w) {
        ...
        if (w.mode === 'content' || w.mode === 'both') {
            if (el.textContent.trim() !== w.content) return 'changed';
        }
        if (w.mode === 'style' || w.mode === 'both') {
            if (w.styles) {
        ...
        openModal('auto-refresh-watch-inspector', `Watch Inspector`, (panel, close) => {
            ...
            function renderDetail() {
                ...
                refreshBtn.addEventListener('click', () => {
                    ...
                    if (w.mode === 'content' || w.mode === 'both') w.content = el.textContent.trim();
                ...
                // Content section
                if (w.mode === 'content' || w.mode === 'both') {
                    const live = el ? el.textContent.trim() : null;
    ...
    function openSettingsMenu() {
        ...
        openModal('auto-refresh-settings', 'Auto Refresh Settings', (panel, close) => {
            ...
            panel.appendChild(makeOptionBtn('⌨ Keyboard Shortcut', false, () => {
                close(); openHotkeyPicker(openSettingsMenu);
            ...
            footer.textContent = `Press ${currentHotkey} to open this menu`;
            panel.appendChild(footer);
