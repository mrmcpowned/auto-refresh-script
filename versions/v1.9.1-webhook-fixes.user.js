...
(function () {
    ...
    const STORAGE_KEY_WEBHOOK = 'autoRefreshWebhook';
    ...
    function buildCss() {
        ...
        .ar-opt-value{color:${T.textDim};font-size:12px;margin-left:12px}
        .ar-subtitle-sm{font-size:11px;color:${T.textDim};margin-top:2px}
        .ar-btn-active .ar-subtitle-sm{color:inherit;opacity:0.85}
        .ar-empty-state{color:${T.textFaint};font-size:13px;padding:8px 0}
        .ar-truncate{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .ar-scroll-container{max-height:60vh;overflow-y:auto}
        .ar-section-hdr-sep{margin-top:16px;border-top:1px solid ${T.borderMid};padding-top:12px}
        .ar-link-btn-center{text-align:center;width:100%}
        .ar-btn-accent{padding:8px 14px;border:none;border-radius:6px;background:${T.accent};color:${T.textLight};cursor:pointer;font:14px system-ui,sans-serif;transition:background .15s}
        .ar-btn-accent:hover{background:${T.accentHover}}
        ...
        .ar-divider{border:none;border-top:1px solid ${T.borderMid};margin:10px 0 6px}
        .ar-split-layout{display:flex;gap:12px}
        .ar-inspector-sidebar{flex:0 0 25%;min-width:130px;border-right:1px solid ${T.borderMid};padding-right:10px;max-height:60vh;overflow-y:auto}
        .ar-detail-pane{flex:1;min-width:0;max-height:60vh;overflow-y:auto}
        .ar-diff-box{background:${T.bgDark};border:1px solid ${T.err};border-radius:6px;padding:8px 10px;font-size:13px;word-break:break-all;max-height:200px;overflow-y:auto}
        .ar-diff-old{color:${T.errLight};text-decoration:line-through}
        .ar-diff-new{color:${T.okText}}
        .ar-input-group{display:flex;align-items:stretch;gap:0}
        .ar-input-group-field{background:${T.bgDark};border:1px solid ${T.border};border-radius:6px 0 0 6px;padding:8px 10px;font:13px monospace;color:${T.text};word-break:break-all;flex:1}
        .ar-input-group-btn{background:${T.bgMid};border:1px solid ${T.border};border-left:none;border-radius:0 6px 6px 0;cursor:pointer;padding:8px 10px;font-size:13px;color:${T.text}}
        .ar-textarea{width:100%;min-height:120px;padding:8px 10px;border:1px solid ${T.borderLight};border-radius:6px;background:${T.bgLight};color:${T.text};font:13px monospace;outline:none;resize:vertical;transition:border-color .2s;box-sizing:border-box}
        .ar-textarea:focus{border-color:${T.accent}}
        .ar-tag-chips{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:8px}
        .ar-tag-chip{background:${T.bgLighter};border:1px solid ${T.border};border-radius:4px;padding:2px 8px;font:11px monospace;color:${T.textMuted};cursor:pointer;transition:background .15s,border-color .15s}
        .ar-tag-chip:hover{background:${T.accent};color:${T.textLight};border-color:${T.accent}}
        *{scrollbar-width:thin;scrollbar-color:${T.borderLight} transparent}
    ...
    function _getWebhookConfig() {
        return GM_getValue(STORAGE_KEY_WEBHOOK, null);
    }
    ...
    function sendWebhook(changeDetails) {
        ...
        const payload = formatWebhookPayload(config.format, changeDetails, config.template);
        ...
        try {
            GM_xmlhttpRequest({
                ...
                onload: (res) => {
                    if (res.status >= 400) console.warn('[AutoRefresh] Webhook failed:', res.status, res.responseText);
                },
                onerror: (err) => {
                    console.warn('[AutoRefresh] Webhook error:', err);
                }
            });
        } catch (err) {
            console.warn('[AutoRefresh] Webhook send error:', err);
        }
    }
    ...
    Modal.define('webhook-config', {
        title: 'Webhook Notifications',
        subtitle: 'Send alerts to external services',
        minWidth: '380px',
        build: (panel, ctx) => {
            const config = _getWebhookConfig() || { url: '', format: 'generic', template: WEBHOOK_DEFAULT_TEMPLATE, enabled: false };
            let currentUrl = config.url || '';
            let currentFormat = config.format || 'generic';
            let currentTemplate = config.template || WEBHOOK_DEFAULT_TEMPLATE;
            let currentEnabled = config.enabled !== false && !!config.url;
            let toggleChanged = false;
            let savedViaButton = false;

            // Re-render settings if toggle was changed and user exits via Back/Escape
            ctx.cleanups.push(() => {
                if (toggleChanged && !savedViaButton) {
                    setTimeout(() => Modal.open('settings'), 0);
                }
            });

            // Enable/Disable toggle
            const toggleRow = h('button', { class: 'ar-toggle-row', style: 'margin-bottom:10px' });
            const toggleLabel = h('div');
            toggleLabel.appendChild(h('div', { text: 'Enabled', style: `font-size:14px;color:${T.text}` }));
            toggleLabel.appendChild(h('div', { class: 'ar-subtitle-sm', text: 'Send webhook when changes are detected' }));

            const trackW = 40, trackH = 22, thumbSize = 18;
            const whToggle = h('div', {
                style: `width:${trackW}px;height:${trackH}px;border-radius:11px;position:relative;transition:background .2s;flex-shrink:0;background:${currentEnabled ? T.accent : T.borderLight}`
            });
            const whThumb = h('div', {
                style: `width:${thumbSize}px;height:${thumbSize}px;border-radius:50%;background:${T.textLight};position:absolute;top:2px;transition:left .2s;box-shadow:0 1px 3px ${T.shadow};left:${currentEnabled ? trackW - thumbSize - 2 + 'px' : '2px'}`
            });
            whToggle.appendChild(whThumb);

            function setWhToggleState(on) {
                whToggle.style.background = on ? T.accent : T.borderLight;
                whThumb.style.left = on ? `${trackW - thumbSize - 2}px` : '2px';
            }

            toggleRow.addEventListener('click', () => {
                currentEnabled = !currentEnabled;
                toggleChanged = true;
                setWhToggleState(currentEnabled);
                // Persist enabled state immediately
                const saved = _getWebhookConfig() || { url: '', format: 'generic', template: WEBHOOK_DEFAULT_TEMPLATE, enabled: false };
                saved.enabled = currentEnabled;
                GM_setValue(STORAGE_KEY_WEBHOOK, saved);
                showToast(currentEnabled ? 'Webhook enabled' : 'Webhook disabled');
            });
            toggleRow.appendChild(toggleLabel);
            toggleRow.appendChild(whToggle);
            ...
            // Template section container (conditionally shown)
            ...
            saveBtn.addEventListener('click', () => {
                ...
                urlInput.style.borderColor = '';
                GM_setValue(STORAGE_KEY_WEBHOOK, {
                    url: currentUrl,
                    format: currentFormat,
                    template: currentTemplate,
                    enabled: currentEnabled && !!currentUrl
                });
                showToast(currentEnabled && currentUrl ? 'Webhook saved' : 'Webhook saved (disabled)');
                Modal.open('settings');
            });
    ...
    Modal.define('settings', {
        ...
        build: (panel, ctx) => {
            ...
            const webhookLabel = webhookConfig && webhookConfig.url ? (webhookConfig.enabled ? 'On' : 'Off') : 'Off';
            panel.appendChild(makeOptionBtn('🌐 Webhook', false, () => ctx.push('webhook-config'),
