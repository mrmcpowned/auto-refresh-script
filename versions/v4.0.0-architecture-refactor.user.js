...
(function () {
    ...
    function buildCss() {
        ...
        *::-webkit-scrollbar-thumb:hover{background:${v('textFaint')}}
        ...
        .ar-picker-highlight{position:absolute;z-index:2147483646;pointer-events:none;border:2px solid ${v('accent')};background:${v('accentFaint')};border-radius:3px;transition:all .05s}
        .ar-picker-label{position:fixed;bottom:12px;left:50%;transform:translateX(-50%);z-index:2147483647;background:${v('bg')};color:${v('text')};font:13px/1 system-ui,sans-serif;padding:8px 16px;border-radius:8px;box-shadow:0 4px 16px ${v('shadowHeavy')}}
    `;
    }
    ...
    function playBeep() {
        ...
        const osc = State.get('audioCtx').createOscillator();
        const gain = State.get('audioCtx').createGain();
        osc.type = 'sine';
        osc.frequency.value = 660;
        gain.gain.setValueAtTime(0.2, State.get('audioCtx').currentTime);
        gain.gain.exponentialRampToValueAtTime(0.01, State.get('audioCtx').currentTime + 0.3);
        osc.connect(gain);
        gain.connect(State.get('audioCtx').destination);
        osc.start();
        osc.stop(State.get('audioCtx').currentTime + 0.3);
    }
        ...
        build: (panel, ctx) => {
            ...
            saveBtn.addEventListener('click', () => {
                ...
                if (!validation.valid) {
                    ...
                    urlInput.style.borderColor = T.err;
                    return;
                }
                urlInput.style.borderColor = '';
                ...
                filteredDomains.forEach(domain => {
                    ...
                    if (domainCount > 1) {
                        ...
                        removeAllBtn.addEventListener('click', () => {
                            ...
                            pages.forEach(pathname => {
                                const pageWatches = group.pages[pathname];
                                pageWatches.forEach(w => WatchStore.removeRemote(w));
                            });
    ...
    function startElementPicker() {
        const highlight = h('div', { class: 'ar-picker-highlight' });
        document.documentElement.appendChild(highlight);

        const label = h('div', {
            class: 'ar-picker-label',
        ...
        for (const w of watches) {
            ...
            if (changes.length > 0) {
                ...
                results.details.push({
                    url: location.href, selector: w.selector, mode: w.mode,
                    old_content: w.content || '', new_content: newContent || el.innerText.trim(),
                    timestamp: new Date().toISOString()
                });
            }
        }
