...
(function () {
    ...
    function buildCss() {
        ...
        *::-webkit-scrollbar-thumb:hover{background:${v('textFaint')}}
        ...
        .ar-toast-undo{pointer-events:auto;display:flex;align-items:center;gap:12px;padding:10px 14px}
        ...
        .ar-toast-progress{position:absolute;bottom:0;left:0;height:3px;background:rgba(255,255,255,.5);border-radius:0 0 8px 8px;transition:width linear}
    ...
    function showUndoToast(message, undoFn, timeout = 5000) {
        ...
        const toast = h('div', { class: 'ar-toast ar-toast-undo', style: 'position:relative' });
        toast.appendChild(h('span', { text: message, style: 'flex:1' }));
                    ...
                    if (domainCount > 1) {
                        ...
                        removeAllBtn.addEventListener('click', () => {
                            ...
                            showUndoToast(t('overview.removedToast', { count: domainCount, domain }), () => {
                                // Restore removed watches by re-adding to their storage keys
                                const byKey = {};
                                removedWatches.forEach(({ watch, storageKey }) => {
                                    if (!byKey[storageKey]) {
                                ...
                                WatchStore.getLocal(); // refresh cache
                                refreshData();
                                renderList();
                                showToast(t('toast.watchesRestored'));
                            });
